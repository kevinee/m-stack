...
int main(void)
{
	...
	memset(buf, 0xa0, EP_2_IN_LEN);

	while (1) {
		
		/* 
		 * This is a very simple loopback test, written
		 * with total disregard of speedy performance!
		 */
        
        /* Loopback EP1_OUT to EP1_IN*/
        ...
        /* Loopback EP2_OUT to EP2_IN*/
        if (usb_is_configured() && usb_out_endpoint_has_data(2)) {
			uint8_t len;
			const unsigned char *data;
			/* Data received from host */
			...
			if (!usb_in_endpoint_halted(2)) {
				...
				len = usb_get_out_buffer(2, &data);
				memcpy(usb_get_in_buffer(2), data, EP_2_IN_LEN);
